...
1. **只播自己发出的**，不代替对方播报。每个 Agent 只负责自己 `sessions_send` 出去的消息。
2. **主动发和回复都算发出**。收到消息后的回复，也是通过 `sessions_send` 发出的，同样需要播报。
...
**行为规则层面（实际跑起来才发现）：**
- **接收方代播报**：一开始以为接收方也要把收到的内容发到群里，其实不对——那样每条消息会被播报两次。正确做法是各播各的，只播自己发出的。
- **回复消息漏播**：规则写"发跨 Agent 消息时播报"容易被理解成"主动发消息时"，导致回复消息漏发。规则必须明确：**任何通过 `sessions_send` 发出的消息，不管是主动发还是回复，都要播报**。
- **越权修改他人 SOUL.md**：发现某个 Agent 漏了规则，直接帮它改 SOUL.md 是越权的。正确做法是通知对方 Agent，让它自己加——或者请 Rose 确认后再改。
