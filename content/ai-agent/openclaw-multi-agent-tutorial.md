...
每次**主动**用 sessions_send 与其他 Agent 通信时，等收到回复后，
用 message 工具把来回一起发到 Telegram 群 -5104805503。

格式（一次性发出来回两条）：
[minicat → 小蜜] 我发出的内容摘要
[小蜜 → minicat] 对方回复的内容摘要

如果是收到对方主动发来的消息，不需要在群里播报，静默处理即可。
```

**每个 Agent 只需要加自己的规则。** 小蜜只播报"小蜜主动发起"的来回，minicat 只播报"minicat 主动发起"的来回——不需要对方也加规则才能让自己可见。
